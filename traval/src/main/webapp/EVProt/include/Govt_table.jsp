...
<body>
 <h5><b><i class="fa fa-dashboard"></i>Goverment Connect</b></h5>
...
<%-- <div class="filter-form">
    ...
    <label for="yearFilter" style="color: black;">Year:</label>
    <select id="yearFilter" name="year"  onchange="loadSearchTravel('Govt_table')">
...
<div class="filter-input">
    ...
    <select id="quarterFilter" name="quarter" onchange="loadSearchTravel('Govt_table')">
...
<div class="filter-input">
    ...
    <select id="monthFilter" name="month" onchange="loadSearchTravel('Govt_table')">
        <option value="All">All</option>
        <%if(quarterfilter.equalsIgnoreCase("Q4")){ %>
        <option value="1"<%=monthfilter.equalsIgnoreCase("1")?"selected":"" %>>January</option>
        <option value="2"<%=monthfilter.equalsIgnoreCase("2")?"selected":"" %>>February</option>
        <option value="3"<%=monthfilter.equalsIgnoreCase("3")?"selected":"" %>>March</option>
        <%} else if (quarterfilter.equalsIgnoreCase("Q1")){%>
        <option value="4"<%=monthfilter.equalsIgnoreCase("4")?"selected":"" %>>April</option>
        <option value="5"<%=monthfilter.equalsIgnoreCase("5")?"selected":"" %>>May</option>
        <option value="6"<%=monthfilter.equalsIgnoreCase("6")?"selected":"" %>>June</option>
        <%}else if (quarterfilter.equalsIgnoreCase("Q2")){%>
        <option value="7"<%=monthfilter.equalsIgnoreCase("7")?"selected":"" %>>July</option>
        <option value="8"<%=monthfilter.equalsIgnoreCase("8")?"selected":"" %>>August</option>
        <option value="9"<%=monthfilter.equalsIgnoreCase("9")?"selected":"" %>>September</option>
        <%} else if  (quarterfilter.equalsIgnoreCase("Q3")){%> 
        <option value="10"<%=monthfilter.equalsIgnoreCase("10")?"selected":"" %>>October</option>
        <option value="11"<%=monthfilter.equalsIgnoreCase("11")?"selected":"" %>>November</option>
        <option value="12"<%=monthfilter.equalsIgnoreCase("12")?"selected":"" %>>December</option>
      <%} %>
    </select>
        ...
        <div class="filter-input">
            ...
            <input type="date" id="fromFilter" name="fromFilter" value="<%=fromDate %>" onchange="loadSearchTravel('Govt_table')" placeholder="Filter by From">
        </div>
        <div class="filter-input">
 ...
 <div class="w3-row w3-section">
    ...
    <table border="1">
      ...
      <%
        ...
        try {
           ...
           if (!fromDate.equalsIgnoreCase("")) {
               query += " AND Date_Format(`date`,'%Y-%m-%d')  >='"+fromDate+"'";
               
           }
           
           if (!toDate.equalsIgnoreCase("")) {
               query += " AND  Date_Format(`date`,'%Y-%m-%d') <= '"+toDate+"'";
                         }
      ...
      %>
            ...
            <td><%= rs.getString("GovtPart") %></td>
            <td><%= rs.getString("GovtMind") %></td>
...
<button onclick="loadLinkContent('NewIndex','contentDiv')">Add new entry</button>
</body>
<script>
...
function loadSearchTravel(pageName){
	...
	let data = {year:year, month:month, quarter:quarter,fromDate:fromDate, toDate:toDate} ;
	//alert(data);
